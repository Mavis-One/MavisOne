...
window.MavisSubscreenRegistry.settings.company = async function renderSettingsCompany(ctx) {
  const { content, data, api, showToast, loadModule, moduleLabels, state, confirmModal } = ctx;
  ...
  content.innerHTML = `
    ...
    ${canManageCompany ? `
    <div class="panel">
      <h3>Configuracoes da empresa</h3>
    ...
    ` : '<div class="panel"><p>Sem permissao para visualizar configuracoes da empresa.</p></div>'}
    ...
    <div class="panel">
      ...
      <form id="userForm" class="form-grid">
        ...
        <div class="row">
          <label>Funcao<select name="role"><option value="user">Usuario</option><option value="admin">Admin</option></select></label>
        </div>
    ...
    <div class="panel">
      ...
      <div class="row" style="margin-bottom: 12px;">
        ...
        <button type="button" class="secondary" id="auditNext">Proximo</button>
      </div>
      <table class="table table-actions">
        <thead><tr><th>Acao</th><th>Alvo</th><th>Por</th><th>Data</th></tr></thead>
  ...
  document.querySelectorAll('.delete-user').forEach((btn) => {
    btn.addEventListener('click', async () => {
      ...
      const confirmed = await confirmModal(`Confirma exclusao do usuario "${username}"?`);
      if (!confirmed) return;
      try {
