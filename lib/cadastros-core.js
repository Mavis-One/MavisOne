...
function counterpartyInUse(data, id) {
  if ((data.finance || []).some((entry) => entry.clientSupplierId === id)) {
    return 'Existem lançamentos financeiros vinculados a este cadastro.';
  }
  if ((data.contacts || []).some((item) => item.personId === id)) {
    return 'Existem contatos vinculados a este cadastro.';
  }
  if ((data.equipments || []).some((item) => item.personId === id)) {
    return 'Existem equipamentos vinculados a este cadastro.';
  }
...
function depositInUse(data, id) {
  if ((data.stockMovements || []).some((m) => m.depositId === id)) {
    return 'Existem movimentações de estoque neste depósito.';
  }
