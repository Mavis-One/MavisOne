...
// O MESMO gerador do resto do sistema (lib/criar-id.js). Aqui havia uma cópia
// própria, ainda com `Math.random().toString(36)`: quando a de lib/db/client.js
// passou para crypto, esta ficou para trás e ninguém notou, porque o teste
// olhava só para a outra. Contato, equipamento, conta bancária e agendamento
// nasciam com id de qualidade diferente do resto — pela mesma função, com o
// mesmo nome, em outro arquivo.
...
// Pessoas/CNPJs e depósitos passaram a ser referenciados por Financeiro,
// Estoque e pelos cadastros novos. Excluir sem checar deixava lançamento,
// movimentação e agendamento apontando para um registro inexistente.
//
// ESTAS DUAS FUNÇÕES SÓ CONHECEM O db.json (fase BB).
// ---------------------------------------------------
// Elas liam também `data.finance`, `data.stockMovements` e `data.equipments` —
// três coleções que foram para o Postgres e que, desde então, chegavam aqui
// VAZIAS. As checagens não falhavam: elas respondiam "ninguém usa", sempre. Um
// depósito com movimentação no razão foi excluído com `success: true`, e o
// razão ficou apontando para um depósito que não existe (há um assim no banco,
// anterior à descoberta — `dep-1786191726703-ssa5yc`, com 3 movimentos).
//
// A parte que mora no banco saiu daqui e virou `contrapartidaEmUso` /
// `depositoEmUso` no server.js, onde `await` é natural e a consulta é uma
// contagem em vez de uma varredura. Este arquivo continua com o que de fato
// mora no arquivo — e é por isso que os nomes ficaram: quem chamar só isto
// recebe só isto, e o server.js é quem junta as duas metades.
function counterpartyInUse(data, id) {
  if ((data.contacts || []).some((item) => item.personId === id)) {
    return 'Existem contatos vinculados a este cadastro.';
  }
  if ((data.tasks || []).some((item) => item.personId === id)) {
    return 'Existem tarefas vinculadas a este cadastro.';
  }
  if ((data.appointments || []).some((item) => item.personId === id)) {
    return 'Existem agendamentos vinculados a este cadastro.';
  }
  return null;
}

function depositInUse(data, id) {
...
const CADASTRO_COLLECTIONS = {
  ...
  // EQUIPAMENTOS FORAM PARA O POSTGRES na fase BB, e a rota generica (que grava
  // por saveData) nao serve mais para eles — ver o cabecalho de
  // lib/db/equipamentos.js. O que sobrou aqui e' o `build`: a montagem e a
  // validacao do formulario, que as rotas proprias reusam em vez de copiar.
  //
  // A entrada continua em CADASTRO_COLLECTIONS de proposito. Tira-la faria as
  // rotas proprias perderem a validacao junto.
  equipments: {
    key: 'equipments',
    prefix: 'equip',
    itemKey: 'equipment',
    ...
    build(body, current, data) {
      ...
      // Fase BB: a garantia deixa de ser uma data digitada.
      //
      // 'prazo' conta N meses a partir da NF-e que vendeu a maquina (ou da data
      // de aquisicao, sem nota); 'data' e' a saida para garantia negociada. Sao
      // MODOS, e nao dois campos convivendo: com os dois valendo, um dia eles
      // discordam e ninguem sabe qual vale.
      const warrantyMode = normalizeStatus(body.warrantyMode ?? current?.warrantyMode, ['prazo', 'data'], 'prazo');
      const mesesCru = body.warrantyMonths ?? current?.warrantyMonths;
      const warrantyMonths = mesesCru === '' || mesesCru === null || mesesCru === undefined
        ? null
        : Math.trunc(toNumber(mesesCru));
      if (warrantyMonths !== null && (warrantyMonths < 0 || warrantyMonths > 600)) {
        throw cadastroError('O prazo de garantia precisa estar entre 0 e 600 meses.');
      }
      // So no modo 'data' a comparacao faz sentido: no modo 'prazo' o
      // `warrantyUntil` que chega do formulario e' resto de um preenchimento
      // anterior, e quem manda e' a conta.
      if (warrantyMode === 'data' && purchaseDate && warrantyUntil && warrantyUntil < purchaseDate) {
      ...
      return {
        ...
        purchaseDate,
        warrantyUntil,
        // Qual NF-e vendeu esta maquina. E' daqui que sai a data de inicio da
        // garantia — a nota e' o documento, e a data de aquisicao e' o que
        // alguem lembrou de digitar.
        nfeId: text(body.nfeId ?? current?.nfeId),
        warrantyMode,
        warrantyMonths,
  ...
  'payment-methods': {
    ...
    build(body, current, data) {
      ...
      // QUEM PROCESSA O CARTÃO (fase BV). O CNPJ da credenciadora vai em
      // pag/detPag/card/CNPJ da NF-e, e sem ele a SEFAZ rejeita com o código
      // 225. Fica na FORMA porque é a forma que representa o contrato: "Cartão
      // de Crédito Rede 2-6x" tem uma credenciadora, uma taxa e um prazo.
      //
      // `data.cardAcquirers` é carregado pela rota (o cadastro mora no
      // Postgres). Sem a lista, a validação sai de cena em vez de recusar tudo:
      // é o mesmo princípio de degradar sem quebrar das outras fases.
      const cardAcquirerId = text(body.cardAcquirerId ?? current?.cardAcquirerId);
      if (cardAcquirerId && Array.isArray(data.cardAcquirers)
        && !data.cardAcquirers.some((a) => a.id === cardAcquirerId)) {
        throw cadastroError('Credenciadora não encontrada.', 404);
      }
      return {
        name,
        code: text(body.code ?? current?.code),
    ...
    serialize(item, data) {
      return {
        ...item,
        bankAccountName: nameById(data.bankAccounts, item.bankAccountId),
        // A lista só existe em `data` quando a rota a carregou (fase BV); fora
        // dali o nome sai vazio em vez de o serializer quebrar.
        cardAcquirerName: nameById(data.cardAcquirers || [], item.cardAcquirerId)
      };
    },
    inUse() {
      return null;
  ...
  'bank-accounts': {
    ...
    build(body, current, data, helpers) {
      ...
      return {
        name,
        // Fase CD: DE QUEM e' a conta. Vazio continua valendo e quer dizer
        // "conta da casa" — toda conta anterior a esta fase esta assim, e
        // inventar um dono para elas mudaria quem as enxerga sem ninguem ter
        // pedido. Ver public/modules/shared/contas_por_estabelecimento.js.
        estabelecimentoId: text(body.estabelecimentoId ?? current?.estabelecimentoId),
