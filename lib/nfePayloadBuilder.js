...
function buildNfeItemPayload(item, index) {
  ...
  if (regra.csosn) {
    ...
    const baseIcms = reducao > 0 ? round2(valorBruto * (1 - reducao / 100)) : valorBruto;
    if (reducao > 0) base.icms_reducao_base_calculo = reducao;
    base.icms_base_calculo = baseIcms;
    base.icms_aliquota = num(regra.aliquotaIcms);
    base.icms_valor = round2(baseIcms * num(regra.aliquotaIcms) / 100);
  }
...
function buildNfePayload({
  ...
  const totalProdutos = round2(items.reduce((soma, i) => soma + num(i.valor_bruto), 0));
