...
/**
 * Apaga uma entrada recém-criada cujo lançamento não se completou.
 *
 * Mesmo motivo do desfazer que já existe dentro de criarEntrada, quando os
 * itens falham: a chave de acesso é ÚNICA em todo o sistema, então uma entrada
 * pela metade queima a chave e a nota nunca mais pode ser lançada — nem por
 * quem está tentando corrigir o erro. Deixar o registro seria pior do que
 * apagar: ele diria que a nota entrou.
 *
 * Isto NÃO contradiz a regra de nunca excluir documento fiscal. Aquela regra é
 * sobre a NF-e que NÓS emitimos, que só pode ser cancelada. Aqui o que se apaga
 * é um lançamento nosso que não chegou a valer; o documento do fornecedor
 * continua existindo, no XML, para ser lançado de novo.
 *
 * Os itens vão junto por `on delete cascade`.
 */
async function desfazerEntrada(id) {
  const { error } = await banco.from('nfe_entrada').delete().eq('id', id);
  assertNoError(error, 'desfazerEntrada');
}

/** Só os sinalizadores. Ver o uso na rota de entrada, quando o financeiro falha. */
async function atualizarSinalizadores(id, { movimentouEstoque, gerouFinanceiro, status }) {
  const campos = {};
  if (movimentouEstoque !== undefined) campos.movimentou_estoque = movimentouEstoque;
  if (gerouFinanceiro !== undefined) campos.gerou_financeiro = gerouFinanceiro;
  if (status !== undefined) campos.status = status;
  if (!Object.keys(campos).length) return null;
  const { error } = await banco.from('nfe_entrada').update(campos).eq('id', id);
  assertNoError(error, 'atualizarSinalizadores');
  return obterEntrada(id);
}

module.exports = {
  desfazerEntrada,
  atualizarSinalizadores,
  buscarPorChave,
  listarEntradas,
  obterEntrada,
  obterXml,
