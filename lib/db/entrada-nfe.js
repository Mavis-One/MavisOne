...
async function criarEntrada({ entrada, itens }) {
  ...
  const { error } = await supabase.from('nfe_entrada').insert(linha);
  ...
  if (linhasItens.length) {
    const { error: erroItens } = await supabase.from('nfe_entrada_item').insert(linhasItens);
    if (erroItens) {
      ...
      await supabase.from('nfe_entrada').delete().eq('id', id);
