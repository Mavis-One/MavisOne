...
const { banco } = require('./client');
// O id do anexo era montado à mão aqui, com Math.random e 6 caracteres — ver
// lib/criar-id.js.
...
/**
 * O QUE PODE ABRIR DENTRO DO NAVEGADOR, E O QUE TEM DE BAIXAR.
 *
 * O anexo era servido SEMPRE com `Content-Disposition: inline` e com o
 * Content-Type que o navegador de quem enviou declarou. Provado nesta base:
 *
 *   arquivo "relatorio.html", tipo "text/html"
 *   -> content-type: text/html
 *   -> content-disposition: inline
 *   -> corpo: <script>alert(document.domain)</script>
 *
 * O arquivo e' entregue pela rota do ERP, entao roda na MESMA ORIGEM da
 * aplicacao: um script ali dentro alcanca o `sessionStorage` (que o navegador
 * copia para a aba nova aberta a partir da pagina) e age como o usuario que
 * abriu o anexo. Qualquer pessoa com o modulo Vendas podia plantar isso num
 * pedido e esperar um administrador clicar.
 *
 * A LISTA E' DE PERMISSAO, e nao de proibicao. Bloquear "text/html" e esquecer
 * "image/svg+xml" (que carrega <script> e e' renderizado como imagem) seria
 * exatamente o tipo de lista que envelhece errado. Aqui, o que nao esta na
 * lista BAIXA em vez de abrir — o pior caso e' um PDF a mais para clicar duas
 * vezes, nao um script rodando.
 */
const TIPOS_QUE_ABREM = new Set([
  'application/pdf',
  'image/png', 'image/jpeg', 'image/gif', 'image/webp', 'image/bmp', 'image/avif'
  // SVG NAO entra: e' XML, aceita <script> e o navegador o executa ao exibir.
]);

function entregaDoAnexo(tipo, nome) {
  const limpo = tipoSeguro(tipo);
  const podeAbrir = TIPOS_QUE_ABREM.has(limpo.toLowerCase());
  return {
    tipo: limpo,
    // RFC 5987 para o nome com acento.
    disposicao: `${podeAbrir ? 'inline' : 'attachment'}; filename*=UTF-8''${encodeURIComponent(nome || 'anexo')}`
  };
}
...
async function enviarAnexo(registroId, arquivo, user) {
  ...
  const { error } = await banco.from('pedido_anexo').insert({
    id,
...
async function removerAnexo(ficha) {
  const { error } = await banco.from('pedido_anexo').delete().eq('id', ficha.id);
...
module.exports = {
  entregaDoAnexo, LIMITE_BYTES, tipoSeguro, enviarAnexo, baixarAnexo, removerAnexo };
