 ...
 *
 *     banco.from('orders').select('*').eq('seller_id', id).order('created_at')
 *
 * e o que sai daqui é:
 *
...
class Consulta {
  ...
  select(colunas = '*', opcoes = {}) {
    ...
    // `head: true` — CONTAR SEM TRAZER AS LINHAS, como no supabase-js.
    //
    // Sem isto, contar custava o preço de ler tudo: `count: 'exact'` já fazia o
    // count(*), mas a consulta de dados saía junto e era ela que doía. O
    // /api/settings chegou a 1 s para devolver 2 KB porque carregava 14.864
    // pedidos e 5.475 produtos — 30 MB de JSON — para escrever cinco números
    // numa tela. O count sozinho responde em milissegundos.
    //
    // Exigir `count` junto é de propósito: um head sem count não pergunta nada
    // e devolveria `{ data: null, count: null }`, que é uma chamada que parece
    // ter funcionado e não trouxe nada — o tipo de bug que se procura no lugar
    // errado por meia hora.
    if (opcoes && opcoes.head) {
      if (!this.estado.contarExato) throw new Error("head: true só faz sentido com count: 'exact' — sem ele a consulta não devolveria nada.");
      this.estado.somenteContagem = true;
    }
    return this;
  }
  ...
  async executar() {
    ...
    try {
      ...
      const querDados = (estado.modo === 'select' || Boolean(estado.retornar)) && !estado.somenteContagem;

      // Em head, a consulta principal NÃO SAI — o count(*) logo abaixo é a
      // única ida ao banco, e é o ponto inteiro da opção. Fora de head ela
      // sempre sai, inclusive para insert/update/delete sem select(): ali o
      // `querDados` decide o que VOLTA, nunca se o comando roda.
      let linhas = [];
      if (!estado.somenteContagem) {
  ...
  // Torna a consulta "aguardável": `await banco.from(...).select()` funciona
