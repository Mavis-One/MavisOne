...
// Consulta crua para as contagens de uso: o razao nao tem construtor de
// consulta aqui, e a pergunta e' um count, nao uma listagem.
...
async function listarClasses({ incluirInativas = false } = {}) {
  let query = banco.from('product_classes').select('*').order('name', { ascending: true });
...
async function criarClasse({ id, name, description, companyId }) {
  const { data, error } = await banco.from('product_classes').insert({
    id,
...
async function atualizarClasse(id, { name, description, active }) {
  ...
  const { data, error } = await banco.from('product_classes').update(campos).eq('id', id).select().single();
  assertSemDuplicata(error, 'atualizarClasse', `Já existe uma classe chamada "${name}".`);
  return mapClasse(data);
}

async function excluirClasse(id) {
  const { error } = await banco.from('product_classes').delete().eq('id', id);
...
async function listarValores(classId, { incluirInativos = false } = {}) {
  let query = banco.from('product_class_values').select('*').order('name', { ascending: true });
...
async function criarValor({ id, classId, name, code, metadata }) {
  const { data, error } = await banco.from('product_class_values').insert({
    id,
    class_id: classId,
    name: String(name || '').trim(),
...
async function atualizarValor(id, { name, code, metadata, active }) {
  ...
  if (active !== undefined) campos.active = Boolean(active);
  const { data, error } = await banco.from('product_class_values').update(campos).eq('id', id).select().single();
  assertSemDuplicata(error, 'atualizarValor', `Já existe "${name}" nesta classe.`);
  return mapValor(data);
}

/**
 * O RAZAO E A PROVA DE QUE UMA COR FOI USADA (fase BO).
 *
 * A unica protecao que havia era a chave estrangeira de
 * product_class_value_assignments — e a propria tela apaga essa linha quando
 * se desmarca a cor do produto. Bastava desmarcar e excluir: os movimentos de
 * estoque ficavam com class_value_id apontando para um valor que nao existe,
 * e o saldo daquela cor virava um balde sem nome no razao.
 *
 * Movimento de estoque NAO tem chave estrangeira para ca' (a coluna e' text
 * com default ''), entao o banco nao recusa nada: a guarda precisa existir.
 *
 * A saida oferecida e' DESATIVAR, que o proprio catalogo ja suporta — valor
 * inativo some da escolha e o movimento antigo continua legivel.
 */
async function movimentosQueUsamValor(id) {
...
async function excluirValor(id) {
  const movimentos = await movimentosQueUsamValor(id);
  if (movimentos > 0) {
    const erro = new Error(
      `${movimentos === 1 ? 'Existe 1 movimentação' : `Existem ${movimentos} movimentações`} de estoque `
      + 'com este valor. Desative-o em vez de excluir: assim ele some do formulário '
      + 'e o histórico continua explicável.'
    );
    erro.status = 409;
    throw erro;
  }
  const { error } = await banco.from('product_class_values').delete().eq('id', id);
...
async function classesDoProduto(productId) {
  const [atribuicoes, valores, catalogo, valoresCatalogo] = await Promise.all([
    banco.from('product_class_assignments').select('*').eq('product_id', productId).eq('active', true),
    banco.from('product_class_value_assignments').select('*').eq('product_id', productId).eq('active', true),
    banco.from('product_classes').select('*'),
    banco.from('product_class_values').select('*')
  ]);
...
async function definirClassesDoProduto(productId, classes) {
  ...
  const apagarValores = await banco.from('product_class_value_assignments').delete().eq('product_id', productId);
  assertNoError(apagarValores.error, 'definirClassesDoProduto/limparValores');
  const apagarClasses = await banco.from('product_class_assignments').delete().eq('product_id', productId);
  ...
  const inserirClasses = await banco.from('product_class_assignments').insert(linhasClasse);
  ...
  if (linhasValor.length) {
    const inserirValores = await banco.from('product_class_value_assignments').insert(linhasValor);
...
async function produtosQueUsam(classId) {
  const { data, error } = await banco.from('product_class_assignments')
...
/**
 * O nome de um valor de classe, para mensagem de erro.
 *
 * Devolve o proprio id quando nao acha — mensagem com id feio e melhor do que
 * mensagem que estoura, e o caso so acontece com dado ja inconsistente.
 */
async function nomeDoValor(id) {
  if (!id) return '';
  const { data, error } = await banco.from('product_class_values').select('name').eq('id', id).maybeSingle();
  if (error || !data) return String(id);
