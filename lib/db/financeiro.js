...
function mapBankAccountRow(row) {
  if (!row) return null;
  return { id: row.id, name: row.name, bank: row.bank || '', agency: row.agency || '', number: row.number || '', createdAt: row.created_at };
}
...
async function createBankAccount(payload) {
  const row = { id: createId('bank'), name: payload.name, bank: payload.bank || '', agency: payload.agency || '', number: payload.number || '', created_at: new Date().toISOString() };
