...
async function createFinancialEntry(payload) {
  ...
  if (error && (error.code === '23503' || /foreign key/i.test(error.message || '')) && /nfe_id/.test(error.message || '')) {
    const err = new Error(
      ...
      'Se a migração supabase/migrations/fase-n-nfe-no-supabase.sql ainda não foi rodada, rode-a; ' +
...
async function getAllFinancialPayments() {
  const { data, error } = await supabase.from('financial_payments').select('*').order('created_at', { ascending: true });
...
async function getNfes() {
  const { data: rows, error } = await supabase.from('nfes').select('*').order('date', { ascending: false });
