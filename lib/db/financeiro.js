...
async function createFinancialEntry(payload) {
  ...
  const row = {
    id,
    code: await proximoCodigoDeLancamento(),
    type: payload.type,
    date: payload.date,
...
// ----------------------------------------------------------------------------
// NF-e + itens
// Sem transação multi-tabela: se a inserção dos itens falhar depois da NF-e já
// ter sido gravada, a NF-e é removida (ação compensatória) para não deixar um
// registro "órfão" sem itens.
//
// A limitação era do cliente supabase-js, que falava HTTP e por isso não tinha
// como manter uma transação aberta entre duas chamadas. Desde a saída do
// Supabase o sistema fala SQL direto, e uma transação de verdade (BEGIN/COMMIT
// numa conexão só) passou a ser POSSÍVEL — a compensação continua aqui por não
// ter sido reescrita, não por impedimento. Ela funciona; é só mais frágil que
// um ROLLBACK, porque a própria remoção compensatória pode falhar.
// ----------------------------------------------------------------------------
...
async function getNfeById(id) {
  const { data: nfeRow, error: nfeError } = await banco.from('nfes').select('*').eq('id', id).maybeSingle();
  assertNoError(nfeError, 'getNfeById');
  if (!nfeRow) return null;
  const { data: items, error: itemsError } = await banco.from('nfe_items').select('*').eq('nfe_id', id);
  assertNoError(itemsError, 'getNfeById(items)');
  return mapNfeRow(nfeRow, items);
}

// Duas consultas, não uma por nota. A versão anterior chamava getNfeById() para
// cada linha: 1 + N viagens de rede, o que só não doía porque nada usava esta
// função. A partir da Fase N ela roda em toda listagem de NF-e.
async function getNfes() {
  const { data: rows, error } = await banco.from('nfes').select('*').order('date', { ascending: false });
  ...
  const { data: itens, error: erroItens } = await banco
    .from('nfe_items')
    .select('*')
    .in('nfe_id', rows.map((row) => row.id));
  assertNoError(erroItens, 'getNfes(items)');

  const itensPorNfe = new Map();
  (itens || []).forEach((item) => {
    if (!itensPorNfe.has(item.nfe_id)) itensPorNfe.set(item.nfe_id, []);
    itensPorNfe.get(item.nfe_id).push(item);
  });

  return rows.map((row) => mapNfeRow(row, itensPorNfe.get(row.id) || []));
}
...
async function createNfe(payload, items) {
  ...
  const { error: nfeError } = await banco.from('nfes').insert(row);
  ...
  if (itemRows.length) {
    const { error: itemsError } = await banco.from('nfe_items').insert(itemRows);
    if (itemsError) {
      await banco.from('nfes').delete().eq('id', id);
...
async function updateNfe(id, payload) {
  const row = { updated_at: new Date().toISOString() };
  if (payload.status !== undefined) row.status = payload.status;
  const { error } = await banco.from('nfes').update(row).eq('id', id);
