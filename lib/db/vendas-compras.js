...
let avisouFaseH = false;

// Se a migração da Fase H ainda não foi aplicada, o PostgREST responde
// PGRST204/42703 ("column not found"). Em vez de derrubar a gravação inteira —
// o usuário perderia o pedido que acabou de digitar — repete sem as colunas
// novas e avisa uma vez no log. Assim que o SQL for rodado, volta ao normal
// sozinho, sem precisar reiniciar.
...
async function withFaseHFallback(row, executar, contexto) {
  let resultado = await executar(row);
  if (ehErroDeColunaInexistente(resultado.error)) {
    if (!avisouFaseH) {
      avisouFaseH = true;
      console.warn(
        `[${contexto}] Colunas da Fase H ausentes no Supabase — gravando sem elas.\n` +
        '  Descontos combinados, frete, despesas e comissões NÃO serão persistidos até a migração ser aplicada.\n' +
        '  Rode o bloco "Fase H" de supabase/schema.sql no SQL Editor do Supabase.'
      );
    }
    const reduzido = { ...row };
    COLUNAS_FASE_H.forEach((coluna) => delete reduzido[coluna]);
...
async function createOrder(payload) {
  ...
  const { data, error } = await withFaseHFallback(row, (r) => supabase.from('orders').insert(r).select().single(), 'createOrder');
...
async function updateOrder(id, payload) {
  ...
  const { data, error } = await withFaseHFallback(row, (r) => supabase.from('orders').update(r).eq('id', id).select().single(), 'updateOrder');
...
async function createQuote(payload) {
  ...
  const { data, error } = await withFaseHFallback(row, (r) => supabase.from('quotes').insert(r).select().single(), 'createQuote');
...
async function updateQuote(id, payload) {
  ...
  const { data, error } = await withFaseHFallback(row, (r) => supabase.from('quotes').update(r).eq('id', id).select().single(), 'updateQuote');
