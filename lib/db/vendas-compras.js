...
function mapOrderQuoteRow(row) {
  ...
  return {
    ...
    // Fase AH. Lista vazia é pedido gravado antes dela: a normalização cria o
    // grupo padrão na leitura e nenhum item fica sem lugar.
    productGroups: Array.isArray(row.product_groups) ? row.product_groups : [],
...
function buildOrderQuoteRow(type, payload) {
  return {
    ...
    ...buildFasePRow(payload),
    ...buildFaseAHRow(payload)
  };
}
...
const COLUNAS_FASE_AH = ['product_groups'];

const COLUNAS_FASE_P = ['nfe_id'];

function buildFaseAHRow(payload) {
  return { product_groups: payload.productGroups || [] };
}
...
const FASES_OPCIONAIS = [
  { nome: 'Fase AH', colunas: COLUNAS_FASE_AH, perda: 'Os grupos de produtos (os itens continuam gravados, todos num grupo só)' },
