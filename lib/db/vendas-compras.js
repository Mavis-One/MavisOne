...
/**
 * OS PEDIDOS PARA QUEM SÓ VAI SOMAR E CONTAR (fase CM).
 *
 * `orders` tem ~60 colunas e a tabela ocupa 104 MB: em média 1.024 bytes por
 * linha. `select *` nas 14.864 traz ~15 MB para o Node, e foi isso que fez a
 * tela Início custar 596 ms para devolver 1 KB — 519 deles só no getOrders.
 *
 * Medido em 17/09/2026 neste banco, mediana de cinco execuções:
 *
 *   select *                      318 ms
 *   sem `items` e `attachments`   237 ms   <- 1,3x. NÃO é onde está o peso.
 *   as oito colunas daqui          39 ms   <- 8,2x
 *
 * Os 10 MB que sobram depois de tirar o `items` estão espalhados em ~60 colunas
 * de texto e numeric. Ou seja: o ganho vem de escolher POUCAS colunas, e não de
 * excluir a mais gorda — foi a primeira coisa que eu tentei, e rendeu 1,3x.
 *
 * O QUE ESTE CONJUNTO COBRE, e por que ele não deforma nenhuma conta:
 *
 *   date, total_amount, amount -> a soma e a série do gráfico (buildSalesChartSeries
 *                                 e kpiFaturamento leem `totalAmount ?? amount`)
 *   seller_id .................. o recorte por vendedor (escopoDeVendas)
 *   status ..................... faturados e pendentes (salesStatus)
 *   id, code, type ............. identidade do registro
 *   nfe_id, created_at ......... o alerta "faturado sem NF-e" do painel de
 *                                pendências (lib/atencao.js/pedidosSemNota)
 *
 * `nfe_id` está aqui por um erro que eu quase cometi: sem ele, `p.nfeId` chega
 * `undefined`, o `if (p.nfeId) return false` nunca filtra, e o alerta passaria a
 * contar como "faturado sem nota" TODO pedido que já tem nota. Alerta errado é
 * pior que alerta ausente — e nada quebraria para avisar.
 *
 * `items` FICA DE FORA e isso é seguro por CONSTRUÇÃO, não por sorte com os
 * dados de hoje: `mapOrderQuoteRow` devolve sempre `totalAmount` como número
 * (`Number(row.total_amount ?? row.amount ?? 0)`), então o
 * `typeof record.totalAmount === 'number'` de serializeSalesRecord é sempre
 * verdadeiro e o fallback que somaria os itens é inalcançável para qualquer
 * registro que passe pelo mapper. Conferido também nos dados: as 14.864 linhas
 * têm `total_amount` preenchido.
 *
 * NÃO USE ISTO PARA MOSTRAR PEDIDO. Os campos que faltam voltam vazios do
 * serializer — cliente, empresa, depósito, frete, desconto. É carregador de
 * agregado, e o nome diz isso.
 */
const COLUNAS_DE_AGREGADO = 'id, code, type, date, created_at, amount, total_amount, status, seller_id';

// `nfe_id` SÓ EXISTE EM `orders`, e não é descuido do schema: orçamento não
// emite nota. Uma lista única para as duas tabelas quebra o orçamento com
// "column quotes.nfe_id does not exist" — foi o que aconteceu quando escrevi
// isto com uma constante só, e o sintoma foi a rota inteira virando
// "Erro interno no servidor (código ...)".
const COLUNAS_DE_AGREGADO_PEDIDO = `${COLUNAS_DE_AGREGADO}, nfe_id`;

async function getOrdersParaAgregado() {
  const { data, error } = await banco.from('orders').select(COLUNAS_DE_AGREGADO_PEDIDO).order('code', { ascending: false });
  assertNoError(error, 'getOrdersParaAgregado');
  return (data || []).map(mapOrderQuoteRow);
}

/** Orçamentos, mesmo recorte e mesmo motivo. Ver getOrdersParaAgregado. */
async function getQuotesParaAgregado() {
  const { data, error } = await banco.from('quotes').select(COLUNAS_DE_AGREGADO).order('code', { ascending: false });
  assertNoError(error, 'getQuotesParaAgregado');
  return (data || []).map(mapOrderQuoteRow);
}

/**
 * O AGREGADO MAIS QUEM É O CLIENTE (fase CM).
 *
 * A diferença em relação ao de cima é uma pergunta: o Painel de Vendas não só
 * soma e conta — ele LISTA os pedidos de cada vendedor, e cada linha mostra o
 * nome do cliente. São seis campos por linha (id, code, customer, amount, date,
 * status), e o `customer` é o único que o recorte puro não consegue produzir:
 * `serializeSalesRecord` o resolve a partir de `clientSupplierId` (buscando no
 * cadastro) e cai em `clientSupplierName` ou `customer` quando não acha.
 *
 * Três colunas a mais, e o preço delas é pequeno. Medido, mediana de cinco:
 *
 *   select *            322 ms
 *   10 colunas           60 ms
 *   13 colunas (esta)    76 ms   <- 4,2x
 *
 * `items` continua fora, e aqui isso tem um limite que vale escrever: quem
 * precisa dos ITENS de cada pedido não pode usar este carregador. É o caso do
 * Relatório de Vendas, que explode cada registro em uma linha por item
 * (relatoriosVendas.linhasDoRegistro) — lá o `select *` não é desperdício, é o
 * que o relatório relata.
 */
const COLUNAS_DE_CLIENTE = 'client_supplier_id, client_supplier_name, customer';

async function getOrdersResumidos() {
  const { data, error } = await banco.from('orders')
    .select(`${COLUNAS_DE_AGREGADO_PEDIDO}, ${COLUNAS_DE_CLIENTE}`)
    .order('code', { ascending: false });
  assertNoError(error, 'getOrdersResumidos');
  return (data || []).map(mapOrderQuoteRow);
}

/** Orçamentos, sem `nfe_id` — a coluna não existe em `quotes`. */
async function getQuotesResumidos() {
  const { data, error } = await banco.from('quotes')
    .select(`${COLUNAS_DE_AGREGADO}, ${COLUNAS_DE_CLIENTE}`)
    .order('code', { ascending: false });
  assertNoError(error, 'getQuotesResumidos');
  return (data || []).map(mapOrderQuoteRow);
}

/**
 * QUANTOS pedidos existem — sem trazer nenhum.
 *
 * `getOrders().length` custava 409 ms e 27,7 MB de JSON neste banco, para a
 * tela de Configurações escrever "14.864" num cartão. Contar custa
 * milissegundos e nenhuma memória. Ver o `head: true` em lib/db/consulta.js.
 */
async function contarOrders() {
  const { count, error } = await banco.from('orders').select('*', { count: 'exact', head: true });
  assertNoError(error, 'contarOrders');
  return count || 0;
}

/** Compras que contam para o total da tela: cancelada não é compra. */
async function contarPurchasesAtivas() {
  const { count, error } = await banco.from('purchases').select('*', { count: 'exact', head: true }).neq('status', 'cancelada');
  assertNoError(error, 'contarPurchasesAtivas');
  return count || 0;
}
...
module.exports = {
  ...
  // Fase CM — para quem só soma e conta, e para quem também lista pedido de
  // forma compacta. Ver o cabeçalho deles.
  getOrdersParaAgregado, getQuotesParaAgregado,
  getOrdersResumidos, getQuotesResumidos,
  getNextSalesCode,
