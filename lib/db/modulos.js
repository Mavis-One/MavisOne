...
const RECURSOS = {
  ...
  'pcp/orders': {
    ...
    campos: {
      ...
      // cadastrado pela empresa, que só rotula. Ver fase-w no supabase/.
...
async function obter(recurso, id) {
  const def = descritor(recurso);
  const { data, error } = await supabase.from(def.tabela).select('*').eq('id', id).maybeSingle();
...
async function criar(recurso, payload) {
  ...
  const { error } = await supabase.from(def.tabela).insert(linha);
...
async function atualizar(recurso, id, payload) {
  ...
  const { error } = await supabase.from(def.tabela).update(linha).eq('id', id);
