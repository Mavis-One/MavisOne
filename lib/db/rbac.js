...
function avisarMigracaoPendente(contexto) {
  ...
  console.warn(
    ...
    '  Rode supabase/migrations/fase-l-controle-de-acesso.sql no SQL Editor do Supabase\n' +
...
async function carregarAcessoDoUsuario(userId) {
  ...
  const [papeis, diretas] = await Promise.all([
    supabase.from('user_roles').select('role_slug').eq('user_id', userId),
    supabase.from('user_permissions').select('permission_slug, effect').eq('user_id', userId)
  ]);
...
async function listarPermissoes() {
  const { data, error } = await supabase.from('permissions').select('*').order('slug', { ascending: true });
...
async function definirPapeisDoUsuario(userId, slugs, concedidoPor) {
  const { error: erroApagar } = await supabase.from('user_roles').delete().eq('user_id', userId);
  ...
  if (linhas.length) {
    const { error } = await supabase.from('user_roles').insert(linhas);
...
async function definirPermissoesDoUsuario(userId, excecoes) {
  const { error: erroApagar } = await supabase.from('user_permissions').delete().eq('user_id', userId);
  ...
  if (linhas.length) {
    const { error } = await supabase.from('user_permissions').insert(linhas);
...
async function definirPermissoesDoPapel(roleSlug, slugs) {
  const { error: erroApagar } = await supabase.from('role_permissions').delete().eq('role_slug', roleSlug);
  ...
  if (linhas.length) {
    const { error } = await supabase.from('role_permissions').insert(linhas);
...
async function listarAcessos({ limite = 100, usuario = '', resultado = '', acao = '' } = {}) {
  let consulta = supabase.from('access_logs').select('*').order('created_at', { ascending: false }).limit(Math.min(500, Math.max(1, limite)));
