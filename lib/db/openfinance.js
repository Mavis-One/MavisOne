// Acesso a dados do módulo Open Finance (tabelas da Fase E em supabase/schema.sql).
...
async function getInstitutions(provider) {
  let query = supabase.from('financial_institutions').select('*').order('name', { ascending: true });
...
async function upsertInstitution(payload) {
  const { data, error } = await supabase.from('financial_institutions').upsert({
...
async function getConnections(estabelecimentoId) {
  let query = supabase.from('open_finance_connections').select('*').order('created_at', { ascending: false });
...
async function getConnectionById(id) {
  const { data, error } = await supabase.from('open_finance_connections').select('*').eq('id', id).maybeSingle();
...
async function createConnection(payload) {
  ...
  const { data, error } = await supabase.from('open_finance_connections').insert(fields).select().single();
...
async function updateConnection(id, payload) {
  ...
  const { data, error } = await supabase.from('open_finance_connections').update(fields).eq('id', id).select().single();
...
async function getConnectionCredentials(id) {
  const { data, error } = await supabase.from('open_finance_connections').select('credentials_encrypted').eq('id', id).maybeSingle();
...
async function getCardsByConnection(connectionId) {
  const { data, error } = await supabase.from('bank_cards').select('*').eq('connection_id', connectionId).order('created_at', { ascending: true });
  assertNoError(error, 'getCardsByConnection');
  return (data || []).map(mapCardRow);
}

async function upsertCard(payload) {
  const { data, error } = await supabase.from('bank_cards').upsert({
...
async function getCardTransactions(cardId) {
  const { data, error } = await supabase.from('card_transactions').select('*').eq('card_id', cardId).order('date', { ascending: false });
...
async function upsertCardTransaction(payload) {
  const { data, error } = await supabase.from('card_transactions').upsert({
...
async function recordWebhookEvent(payload) {
  const { data, error } = await supabase.from('open_finance_webhook_events').insert({
    provider: payload.provider,
  ...
  if (error && error.code === '23505' && payload.providerEventId) {
    const { data: existente, error: buscaError } = await supabase.from('open_finance_webhook_events')
      .select('*')
...
async function markWebhookEventProcessed(id, error) {
  const { data, error: dbError } = await supabase.from('open_finance_webhook_events').update({
    processed: !error,
    error: error || null
...
async function recordAuditLog(payload) {
  const { data, error } = await supabase.from('open_finance_audit_logs').insert({
    connection_id: payload.connectionId || null,
...
async function getAuditLogs({ connectionId, limit } = {}) {
  let query = supabase.from('open_finance_audit_logs').select('*').order('created_at', { ascending: false }).limit(limit || 200);
