...
async function getNextCadastroCode() {
  const { data, error } = await supabase.rpc('next_cadastro_code');
...
async function getCnpjs() {
  const { data, error } = await supabase.from('cnpjs').select('*').order('created_at', { ascending: false });
  assertNoError(error, 'getCnpjs');
  return (data || []).map(mapCnpjRow);
}

async function getCnpjById(id) {
  const { data, error } = await supabase.from('cnpjs').select('*').eq('id', id).maybeSingle();
...
async function createCnpj(payload) {
  ...
  const { error } = await supabase.from('cnpjs').insert(row);
...
async function updateCnpj(id, payload) {
  ...
  const { error } = await supabase.from('cnpjs').update(row).eq('id', id);
  assertNoError(error, 'updateCnpj');
  return getCnpjById(id);
}

async function deleteCnpj(id) {
  const { error } = await supabase.from('cnpjs').delete().eq('id', id);
...
async function updateDeposit(id, payload) {
  ...
  const { data, error } = await supabase.from('deposits').update(row).eq('id', id).select().maybeSingle();
