...
async function lerLinha() {
  const { data, error } = await supabase.from('crm_connection').select('*').eq('id', 1).maybeSingle();
...
async function salvarConexao(payload) {
  ...
  const { error } = await supabase.from('crm_connection').upsert(linha);
...
async function testarConexao() {
  ...
  const { error } = await supabase.from('crm_connection').upsert({
    id: 1,
