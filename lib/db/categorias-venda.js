...
async function pedidosQueUsam(nome) {
  const { rows } = await consultar(
    'select count(*)::int as n from orders where lower(trim(category)) = lower(trim($1))',
    [String(nome || '')]
  );
  return rows[0].n;
}
...
async function excluir(id) {
  ...
  const emUso = await pedidosQueUsam(categoria.name);
  if (emUso > 0) {
    const erro = new Error(
      `${emUso} ${emUso === 1 ? 'pedido usa' : 'pedidos usam'} a categoria "${categoria.name}". `
      + 'Marque como inativa em vez de excluir: assim ela some do formulário e os pedidos continuam explicáveis.'
    );
    erro.status = 409;
    throw erro;
  }
...
module.exports = { listar, obter, criar, atualizar, excluir, pedidosQueUsam };
