...
// A tabela `tBand` vem do catálogo compartilhado, que a TELA também carrega.
// Ela nasceu aqui na fase BV e foi copiada à mão para o formulário; na BW
// ganhou um terceiro consumidor (o select de bandeira na linha de pagamento do
// pedido) e virou arquivo próprio. Duas listas do mesmo assunto divergem no dia
// em que alguém acrescenta uma bandeira só de um lado.
const bandeiraCartao = require('../../public/modules/shared/bandeira_cartao');

const BANDEIRAS = bandeiraCartao.CATALOGO;
...
function normalizar(payload) {
  ...
  const brands = (Array.isArray(payload.brands) ? payload.brands : [])
    .map((b) => String(b || '').trim())
    .filter((b) => bandeiraCartao.existe(b));
