...
/**
 * O nome do cliente de um registro de venda.
 *
 * Existe como função, e não como expressão na linha, porque a escolha tem três
 * partes e cada uma responde a um jeito diferente de o dado chegar aqui. O
 * porquê de cada uma está em `linhasDoRegistro`, onde ela é usada.
 */
function nomeDeCliente(registro) {
  const resolvido = texto(registro.customer);
  // `serializeSalesRecord` publica '-' quando não há cliente. Aqui isso é vazio.
  if (resolvido && resolvido !== '-') return resolvido;
  return texto(registro.clientSupplierName);
}
...
function linhasDoRegistro(registro) {
  ...
  return itens.map((item, i) => {
    ...
    return {
      ...
      // O NOME DO CLIENTE VEM DE `customer`, e a ordem importa (fase CQ).
      //
      // O DEFEITO QUE ISTO CONSERTA: este campo lia `clientSupplierName`, que o
      // servidor NÃO entrega. A rota monta os registros com
      // `serializeSalesRecord`, e ele resolve o nome — busca no cadastro pelo
      // `clientSupplierId` e cai em `clientSupplierName` ou `customer` quando
      // não acha — publicando o resultado em `customer` e só nele. Resultado
      // medido: TODAS as linhas do Relatório de Vendas diziam "Sem cliente",
      // inclusive as de pedido com cliente vinculado, e o filtro por texto
      // deixava de achar qualquer venda pelo nome do cliente.
      //
      // `customer` PRIMEIRO, e não como reserva, porque ele é o nome resolvido
      // no cadastro: `clientSupplierName` é a cópia gravada no pedido, que
      // envelhece quando o cliente é renomeado. A reserva fica para quem chamar
      // esta função com linha crua do banco em vez de registro serializado.
      //
      // `'-'` é tratado como vazio: `serializeSalesRecord` usa esse traço como
      // marca de "sem cliente", e sem esta conversão o relatório trocaria
      // "Sem cliente" por um traço solto, que é pior — parece coluna quebrada.
      clienteNome: nomeDeCliente(registro) || 'Sem cliente',
 ...
 */
// Quantas linhas de detalhe cada vendedor leva na resposta.
//
// OS INDICADORES SAEM DE TODAS AS LINHAS; SÓ O DETALHE É RECORTADO. Contar o
// faturamento de uma amostra seria mentir no número — o recorte é só do que vai
// para a tela como tabela.
//
// Medido no navegador, Relatórios > Relatório por Vendedor com os dados reais:
//
//   /api/reports/vendas ..... 9.642 KB de resposta   (a tabela da tela usa 25 linhas)
//   DOM ..................... 168.170 nós, 17.890 linhas, 17.895 botões
//   tempo ................... 16 s
//
// 168 mil nós é cinco vezes a segunda tela mais pesada do sistema. A tela abria
// — mas abria depois de o navegador montar um <details open> por vendedor com
// todos os itens que ele já vendeu na vida, e é isso que se sente como "a tela
// não carrega". O caminho para ver tudo de um vendedor já existia e é melhor:
// clicar no nome dele no ranking filtra o relatório inteiro por ele.
const LINHAS_POR_VENDEDOR = 10;

function agruparPorVendedor(linhas, { linhasPorVendedor = LINHAS_POR_VENDEDOR } = {}) {
  const mapa = new Map();
  for (const linha of linhas) {
  ...
  return [...mapa.values()]
    .map((grupo) => ({
      ...grupo,
      indicadores: indicadoresDe(grupo.linhas),
      // `linhasNoTotal` vai junto para a tela poder DIZER que recortou. Uma
      // tabela cortada em silêncio é pior do que uma tabela grande: quem soma
      // as linhas na mão acha que o relatório está errado.
      linhasNoTotal: grupo.linhas.length,
      linhas: grupo.linhas.slice(0, Math.max(0, linhasPorVendedor))
    }))
